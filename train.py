...
def train(data_path, lr , weights, epochs, validation_split, validation_path, save_weights_path, plot_data, batch_size):
    ...
    if((validation_split == None) and (validation_path == None)):
        train_low = sorted([file for file in glob(os.path.join(data_path,"low/*")) if not file.split('/')[-1].startswith('.')])
        train_enhanced = sorted([file for file in glob(os.path.join(data_path,"high/*")) if not file.split('/')[-1].startswith('.')])
        ...
        val_low = sorted([file for file in glob(os.path.join(validation_path,"low/*")) if not file.split('/')[-1].startswith('.')])
        val_enhanced = sorted([file for file in glob(os.path.join(validation_path,"high/*")) if not file.split('/')[-1].startswith('.')])
        ...
        MAX_TRAIN_IMAGES = len([file for file in glob(os.path.join(data_path,"low/*")) if not file.split('/')[-1].startswith('.')])
        MAX_TRAIN_IMAGES = MAX_TRAIN_IMAGES - int(MAX_TRAIN_IMAGES * (validation_split/100))

        train_low = sorted([file for file in glob(os.path.join(data_path,"low/*")) if not file.split('/')[-1].startswith('.')])[:MAX_TRAIN_IMAGES]
        train_enhanced = sorted([file for file in glob(os.path.join(data_path,"high/*")) if not file.split('/')[-1].startswith('.')])[:MAX_TRAIN_IMAGES]
        train_dataset = create_dataset(train_low, train_enhanced, batch_size)

        if(validation_split == 0):
            val_dataset = None
        else:
            val_low = sorted([file for file in glob(os.path.join(data_path,"low/*")) if not file.split('/')[-1].startswith('.')])[MAX_TRAIN_IMAGES:]
            val_enhanced = sorted([file for file in glob(os.path.join(data_path,"high/*")) if not file.split('/')[-1].startswith('.')])[MAX_TRAIN_IMAGES:]
            val_dataset = create_dataset(val_low, val_enhanced, batch_size)
